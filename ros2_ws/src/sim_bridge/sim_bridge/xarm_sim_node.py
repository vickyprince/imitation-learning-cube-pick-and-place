...
/sim/camera/wrist/image_compressed   sensor_msgs/CompressedImage   @ 30 Hz  (display only)
...
class XArmSimNode(Node):
    ...
    def _publish_obs(self):
        ...
        ee.pose.position.x       = float(state[0])
        ee.pose.position.y       = float(state[1])
        ee.pose.position.z       = float(state[2])
        ee.pose.orientation.x    = float(state[3])
        ee.pose.orientation.y    = float(state[4])
        ee.pose.orientation.z    = float(state[5])
        ee.pose.orientation.w    = float(state[6])
